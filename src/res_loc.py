...
class Model(nn.Module):
    ...
    def __init__(self, model_path=None):
        ...
        self.condense_block1 = nn.Sequential(
            ConcentratedModule(4096, 1024, 2048, 128),
            AttentionPart(2048)
        )
        
        self.condense_block2 = nn.Sequential(
            ConcentratedModule(2048, 512, 1024, 64),
            AttentionPart(1024)
        )
        ...
        self.outconv = nn.Conv2d(512, 1, 1)
        self.relu = nn.ReLU()
        self.param_init()
    ...
    def forward(self, h, l):
        ...
        feature_map1 = self.condense_block1(feature_map)
        feature_map2 = self.condense_block2(feature_map1)
        feature_map3 = self.condense_block3(feature_map2)
    ...
    def load_model(self, model_path=None):
        ...
        for i in range(3):
            model.layer4[i].conv2.padding = (2, 2)
            model.layer4[i].conv2.dilation = (2, 2)
        if (model_path != None):
            model.load_state_dict(torch.load(model_path))
        features = list([model.conv1, model.bn1, model.relu, model.maxpool]) + list(model.layer1) + list(model.layer2) + list(model.layer3) + list(model.layer4)
